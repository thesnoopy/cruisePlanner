...
class TravelDocumentsSection extends StatefulWidget {
  ...
  final TravelDocumentSectionService? service;

  @override
  State<TravelDocumentsSection> createState() => _TravelDocumentsSectionState();
}
...
class _TravelDocumentsSectionState extends State<TravelDocumentsSection> {
  ...
  Future<void> _reload() async {
    ...
    final data = await _service.loadForTravelItem(widget.travelItemId);
    if (!mounted) {
      return;
    }
    setState(() {
  ...
  Future<void> _attachDocument(DocumentRecord document) async {
    ...
    await _service.attachExistingDocument(
      travelItemId: widget.travelItemId,
      ...
      MaterialPageRoute(
        builder: (_) => UrlSnapshotCaptureScreen(
          target: UrlDocumentTarget(
            type: UrlDocumentTargetType.travelItem,
            id: widget.travelItemId,
          ),
        ),
      ),
    );
